...
type IHttpFetch = <D = any>(
  url: string, data?: any, options?: string | false | IHttpFetchOptions
) => Promise<D | null>
...
restful.forEach(item => {
  ...
  http[item.alias || method] = <D>(
      url: string, data?: { [x: string]: unknown }, options: IHttpFetchOptions = {},
    ) => {
    ...
    return req<IRes<D>>(url, method, data, { ...item.headers })
      .then(resp => {
        ...
        if (!silent) {
          message.error(errorMsg || resp.reason || resp.prompts || resp.message || '服务器开小差了，请您稍后再试')
        }
